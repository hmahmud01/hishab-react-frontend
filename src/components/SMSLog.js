...
class SMSLog extends Component {
	...
	render(){
		const listItems = this.state.data.map(
			(listItem) => 
				<ListItem tid={listItem.tid} phone={listItem.phone} text={listItem.text} date={listItem.date} status={listItem.status} />
		);
	...
	render(){
		return(
			<tr>
				...
				<td>
					<span className={this.props.status === "sent" ? "label label-success" : "label label-danger"}>
						{this.props.status}
					</span>					
				</td>
